            ...
            if (icon) {
              return that.getEntry(icon, { type: 'blob' }, function(error, blob) {
                if (error) {
                  ...
                  console.error(error);
                  return callback(error, apkInfo);
                }

                if (blob) {

                  if (typeof Blob === 'undefined' || typeof document === 'undefined' || typeof URL === 'undefined') {
                    if (that.options.iconType === 'buffer') {
                      apkInfo.icon = blob;
                      callback(null, apkInfo);
                    } else {
                      var error = new Error('unsupported method');
                      console.error(error);
                      callback(error, apkInfo);
                    }
                    ...
                    if (that.options.iconType === 'blob') {
                      ...
                      return utils.blobToBase64(blob, function(error, base64Icon) {
                        if (error) {
